...
static void HotkeyAdjustTargetSpeed(double delta)
{
	...
	gsUpdateFrequency(EmuConfig);
	GetMTGS().SetVSync(EmuConfig.GetEffectiveVsyncMode());
	Host::AddKeyedFormattedOSDMessage("SpeedChanged", 10.0f, "Target speed set to %.1f%%.", EmuConfig.Framerate.NominalScalar * 100.0);
}
...
DEFINE_HOTKEY("IncreaseSpeed", "System", "Decrease Target Speed", [](bool pressed) {
	if (!pressed)
		HotkeyAdjustTargetSpeed(-0.1);
})
