...
{
  const bool enabled = m_ui.enableHWFixes->isEnabled() && m_ui.enableHWFixes->isChecked();
  if (!enabled && m_ui.hardwareRendererGroup->isVisible())
    m_ui.hardwareRendererGroup->setCurrentIndex(0);

  m_ui.hardwareRendererGroup->setTabEnabled(1, enabled);
...
void GraphicsSettingsWidget::updateRendererDependentOptions()
{
  ...
  if (const int index = m_ui.softwareRendererGroup->indexOf(m_ui.advancedTab); index >= 0 && is_hardware)
    m_ui.softwareRendererGroup->removeTab(index);
  if (const int index = m_ui.hardwareRendererGroup->indexOf(m_ui.advancedTab); index >= 0 && is_software)
    m_ui.hardwareRendererGroup->removeTab(index);
  if (const int index = m_ui.hardwareRendererGroup->indexOf(m_ui.advancedTab); index < 0 && is_hardware)
    m_ui.hardwareRendererGroup->insertTab(m_ui.hardwareRendererGroup->count(), m_ui.advancedTab, tr("Advanced"));
  if (const int index = m_ui.softwareRendererGroup->indexOf(m_ui.advancedTab); index < 0 && is_software)
    m_ui.softwareRendererGroup->insertTab(m_ui.softwareRendererGroup->count(), m_ui.advancedTab, tr("Advanced"));
