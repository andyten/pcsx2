  ...
  switch (column)
  {
    ...
    case Column_Serial:
    {
      if (left->serial == right->serial)
        return titlesLessThan(left_row, right_row, ascending);
      return ascending ? (StringUtil::Strcasecmp(left->serial.c_str(), right->serial.c_str()) < 0) :
                         (StringUtil::Strcasecmp(right->serial.c_str(), left->serial.c_str()) > 0);
    }
    ...
    case Column_CRC:
    {
      ...
      return ascending ? (left->crc < right->crc) : (right->crc > left->crc);
    }
...
void GameListModel::loadCommonImages()
{
  ...
  for (u32 i = 0; i < GameList::CompatibilityRatingCount; i++)
